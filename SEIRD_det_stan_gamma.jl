...
model {
    ...
    //priors
    gamma ~ uniform(0.05, 0.25);

    //sampling distribution (uniform)
    //for (t in 1:T)
    //    y[t] ~ uniform(0, u[t,5]);

    //sampling distribution (uniform 2 - diff.)
    //y[1] ~ uniform(0, u[1,5]);
    //for (t in 2:T)
    //    y[t] ~ uniform(0, u[t,5] - u[t-1,5]);
...
function main_prog()
    ...
    alpha = 1/0.5       #inverso do período médio de latência - NetoNature2021
    gamma = 1/9.2       #inverso do tempo médio de recuperação - NetoNature2021 => 0.1087
    ...
    #y = Int64.(round.(rand.(Uniform.(0, sol[5,2:end].-sol[5,1:end-1]))))
    ...
    seird_data = Dict("T" => T, "u0" => u0, "t0" => 0.0, "ts" => t, "y" => y, "N" => N, "beta" => beta, "alpha" => alpha, "eta" => eta)
    #init = Dict("gamma" => gamma)

    # Keep tmpdir across multiple runs to prevent re-compilation
    tmpdir = joinpath(@__DIR__, "tmp")

    sm = SampleModel("sm_seird_gamma", SEIRD_model_gamma, tmpdir)
