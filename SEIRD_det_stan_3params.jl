...
SEIRD_model_beta_alpha_gamma_u0 = "
functions {
...
function main_prog()
    ...
    #Aprender beta, alpha e gamma
    #seird_data = Dict("T" => T, "u0" => u0, "t0" => 0.0, "ts" => t, "D_obs" => D_obs, "N" => N, "eta" => eta)

    #Aprender beta, alpha e eta
    #seird_data = Dict("T" => T, "u0" => u0, "t0" => 0.0, "ts" => t, "D_obs" => D_obs, "N" => N, "gamma" => gamma)

    #Aprender beta, gamma e eta
    seird_data = Dict("T" => T, "u0" => u0, "t0" => 0.0, "ts" => t, "D_obs" => D_obs, "N" => N, "alpha" => alpha)

    # Keep tmpdir across multiple runs to prevent re-compilation
    tmpdir = joinpath(@__DIR__, "tmp")

    #Escolher de acordo com a combinação de parâmetros
    #sm = SampleModel("sm_seird_beta_alpha_gamma_u0", SEIRD_model_beta_alpha_gamma_u0, tmpdir)
    #sm = SampleModel("sm_seird_beta_alpha_eta_u0", SEIRD_model_beta_alpha_eta_u0, tmpdir)
    sm = SampleModel("sm_seird_beta_gamma_eta_u0", SEIRD_model_beta_gamma_eta_u0, tmpdir)
    
    rc = stan_sample(sm; data = seird_data, num_samples = 2000)
    ...
    if success(rc)
        ...
        StatsPlots.savefig("MCMC_3params_u0.png")
    end
end
